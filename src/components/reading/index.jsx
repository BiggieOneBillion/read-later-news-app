...
import { LuLoader, LuAlertCircle, LuCircleAlert } from "react-icons/lu";

const ErrorComponent = ({ retry, fn }) => (
  <div className="min-h-[60vh] flex flex-col items-center justify-center p-8 space-y-6">
    ...
    <div className="text-center space-y-2">
      ...
      <p className="text-sm text-slate-500 max-w-xs mx-auto">We couldn't retrieve this story from the wire. Please check your connection.</p>
    </div>
    <button
      onClick={() => retry()}
...
const LoadingComponent = () => (
  <div className="min-h-[70vh] flex flex-col items-center justify-center gap-6">
    ...
    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] animate-pulse">Loading Article...</p>
  </div>
);
...
const Index = () => {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ["guest-reading", param.topic],
    queryFn: async () => {
  ...
  if (isError) return <ErrorComponent retry={refetch} fn={() => router.push("/")} />;

  const articles = data?.articles || [];
  if (articles.length === 0) return <ErrorComponent retry={refetch} fn={() => router.push("/")} />;
    ...
    <div className="min-h-screen bg-white">
      ...
      <article className="max-w-4xl mx-auto px-4 sm:px-8 py-12 lg:py-20 space-y-12">
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-x-8 gap-y-4 pt-4 border-t border-slate-100">
               <div className="flex items-center gap-2">
                 ...
                 <span className="text-xs font-bold text-slate-900 uppercase tracking-wider">{article.author || "Guest Correspondent"}</span>
               </div>
        ...
        <section className="max-w-2xl mx-auto">
          <div className="prose prose-slate lg:prose-lg text-justify selection:bg-slate-900 selection:text-white">
            <p className="text-lg md:text-xl lg:text-2xl font-medium text-slate-600 leading-relaxed first-letter:text-6xl md:first-letter:text-8xl first-letter:font-black first-letter:text-slate-900 first-letter:mr-4 first-letter:float-left first-letter:leading-none">
               {article.content ? (
                 <>
